 ...
 *      在BeanFactory标准初始化后调用；来定制和修改BeanFactory的内容；
 ...
 *      3)、从容器中获取到所有的BeanDefinitionRegistryPostProcessor类型组件。
 ...
 *      com.annotation.test.IOCTest_Ext$1[source=我发布的事件]
 *      ContextClosedEvent
 *
 ...
 *                      2.否则，同步的方式执行Listener方法；invokeListener(listener,event);
 ...
 *          我们就可以在其它组件要派发事件，自动注入这个applicationEventMulticaster
 *
 * 【容器中有哪些监听器】
 *      1.容器创建对象：refresh();
 *      2. registerListeners();
...
@ComponentScan("com.annotation.ext")
@Configuration
